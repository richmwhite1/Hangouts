...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-black/60 text-[#FAFAFA] flex flex-col gap-6 rounded-2xl border border-[#333333] shadow-sm hover:shadow-md transition-all duration-200",
        className
      )}
      {...props}
    />
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-[#A8A8A8] text-sm leading-relaxed", className)}
      {...props}
    />
  )
}
