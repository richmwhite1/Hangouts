...
export async function POST(request: NextRequest) {
  try {
    ...
    const isParticipant = hangout.participants.some(p => p.userId === user.id)
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const allParticipants = [
        ...
        { userId: user.id } // Add creator as participant
      ]
        ...
        uniqueParticipants.map(participant =>
          tx.PollParticipant.create({
            data: {
              ...
              status: participant.userId === user.id ? 'ACTIVE' : 'INVITED',
              canVote: true,
