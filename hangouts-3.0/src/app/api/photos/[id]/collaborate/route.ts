...
export async function POST(
  ...
  try {
    ...
    const hasAccess = photo.creatorId === user.id || 
                     photo.isPublic || 
    ...
    switch (action) {
      case 'add_comment':
        return await addComment(photoId, user.id, content)
      
      case 'add_reaction':
        return await addReaction(photoId, user.id, content)
      
      case 'add_tag':
        return await addTag(photoId, user.id, tagUserId, x, y)
      
      case 'remove_tag':
        return await removeTag(photoId, user.id, tagUserId)
      
      case 'add_annotation':
        return await addAnnotation(photoId, user.id, content, x, y)
