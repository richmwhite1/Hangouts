...
export default function MessagesModal({ isOpen, onClose, selectedUser }: MessagesModalProps) {
  ...
  const loadFriends = async () => {
    ...
    if (!user?.token) {
      console.log('No user token available for loading friends')
      return
    }

    try {
      setIsLoadingFriends(true)
      console.log('Loading friends with token:', user.token.substring(0, 20) + '...')
      ...
      if (response.friends && Array.isArray(response.friends)) {
        console.log('Found friends:', response.friends.length)
        setFriends(response.friends.map(friend => ({
          id: friend.id,
          name: friend.name,
