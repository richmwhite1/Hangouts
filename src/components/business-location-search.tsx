...
const searchWithNominatim = async (query: string, userLocation?: { lat: number; lng: number }): Promise<BusinessResult[]> => {
  try {
    ...
    const response = await fetch(
      `https://nominatim.openstreetmap.org/search?format=json&q=${searchQuery}&limit=5&addressdetails=1&extratags=1&class=amenity&class=shop&class=leisure&class=tourism${locationParam}`,
      { headers: { 'User-Agent': 'Hangouts3.0/1.0' } }
    )
...
const searchWithSimpleText = async (query: string, userLocation?: { lat: number; lng: number }): Promise<BusinessResult[]> => {
  try {
    ...
    const response = await fetch(
      `https://nominatim.openstreetmap.org/search?format=json&q=${searchQuery}&limit=3&addressdetails=1${locationParam}`,
      { headers: { 'User-Agent': 'Hangouts3.0/1.0' } }
    )
...
const searchWithFallback = async (query: string, userLocation?: { lat: number; lng: number }): Promise<BusinessResult[]> => {
  try {
    // Simple fallback using basic Nominatim search
    const searchQuery = encodeURIComponent(query)
    const locationParam = userLocation ? `&lat=${userLocation.lat}&lon=${userLocation.lng}` : ''
    
    const response = await fetch(
      `https://nominatim.openstreetmap.org/search?format=json&q=${searchQuery}&limit=5&addressdetails=1${locationParam}`,
      {
        headers: {
