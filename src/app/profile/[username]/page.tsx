...
import { apiClient, User, Hangout, Event } from '@/lib/api-client'
...
export default function ProfilePage() {
  ...
  const fetchProfileData = async () => {
    try {
      ...
      // Fetch user stats
      const statsResponse = await fetch(`/api/users/${userResponse.user.id}/stats`)
      if (statsResponse.ok) {
      ...
      const hangoutsResponse = await fetch(`/api/users/${userResponse.user.id}/hangouts`)
  ...
  const handleFriendRequest = async () => {
    ...
    try {
      if (isFriend) {
        // Remove friend
        await apiClient.removeFriend(profileUser.id)
        setIsFriend(false)
      } else if (friendRequestSent) {
        // Cancel friend request
        await apiClient.cancelFriendRequest(profileUser.id)
        setFriendRequestSent(false)
      } else {
        // Send friend request
        await apiClient.sendFriendRequest(profileUser.id)
